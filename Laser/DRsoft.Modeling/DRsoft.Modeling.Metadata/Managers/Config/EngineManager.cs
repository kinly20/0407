...
namespace DRsoft.Modeling.Metadata.Managers.Config
{
    ...
    /// 引擎元数据管理
    /// </summary>
    internal class EngineMetadataManager : BaseManager<EngineConfig>, IMetadataManager
    {
        /// <summary>
        /// 实例
        /// </summary>
        private static volatile EngineMetadataManager s_Instance;
        ...
        public static EngineMetadataManager GetInstance()
        {
            if (s_Instance == null)
            {
                lock (s_LockObject)
                {
                    if (s_Instance == null)
                    {
                        s_Instance = new EngineMetadataManager();
                    }
                }
            }
        ...
        public override Guid Insert(EngineConfig metadata)
        {
        ...
        public override void Update(EngineConfig metadata)
        {
            // 更新最后修改人，最后修改时间
            metadata.ModifiedBy = CurrentUserName;
            metadata.ModifiedOn = Clock.Now;
        ...
        public void Update(string jsonData, bool isSubmit)
        {
            ...
            EngineConfig metadata = JsonConvert.DeserializeObject<EngineConfig>(jsonData, JsonSetting);
            Update(metadata);
        }
        ...
        public void CheckOut(Guid metadataId)
        {
            ...
            {
                // 签出运行时文件，生成设计时文件
                EngineConfig metadata = FileOperator.CheckOut(metadataId);
                // 刷新二开运行时
                EngineConfig runtimeMetadata = metadata.Clone();
                // 更新运行时缓存
        ...
        /// <returns></returns>
        public override MetadataStatusItem GetStatusItem(EngineConfig metadata, StatusType status)
        {
