...
namespace DRsoft.Modeling.Metadata.Managers.Config
{
    ...
    /// 元数据管理
    /// </summary>
    internal class CameraManager : BaseManager<CamConfig>, IMetadataManager
    {
        /// <summary>
        /// 实例
        /// </summary>
        private static volatile CameraManager s_Instance;
        ...
        public override Guid Insert(CamConfig metadata)
        {
        ...
        public override void Update(CamConfig metadata)
        {
            // 更新最后修改人，最后修改时间
            metadata.ModifiedBy = CurrentUserName;
            metadata.ModifiedOn = Clock.Now;
        ...
        public void Update(string jsonData, bool isSubmit)
        {
            ...
            CamConfig metadata = JsonConvert.DeserializeObject<CamConfig>(jsonData, JsonSetting);
            Update(metadata);
        }
        ...
        public void CheckOut(Guid metadataId)
        {
            ...
            {
                // 签出运行时文件，生成设计时文件
                CamConfig metadata = FileOperator.CheckOut(metadataId);
                // 刷新二开运行时
                CamConfig runtimeMetadata = metadata.Clone();
                // 更新运行时缓存
        ...
        /// <returns></returns>
        public override MetadataStatusItem GetStatusItem(CamConfig metadata, StatusType status)
        {
