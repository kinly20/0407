...
namespace DRsoft.Modeling.Metadata.Managers.Config
{
    ...
    /// 元数据管理
    /// </summary>
    internal class VisionCalibrationManager : BaseManager<VisionCalibrationConfig>, IMetadataManager
    {
        /// <summary>
        /// 实例
        /// </summary>
        private static volatile VisionCalibrationManager s_Instance;
        ...
        public static VisionCalibrationManager GetInstance()
        {
            if (s_Instance == null)
            {
                lock (s_LockObject)
                {
                    if (s_Instance == null)
                    {
                        s_Instance = new VisionCalibrationManager();
                    }
                }
            }
            return s_Instance;
        ...
        public override Guid Insert(VisionCalibrationConfig metadata)
        {
        ...
        public override void Update(VisionCalibrationConfig metadata)
        {
            // 更新最后修改人，最后修改时间
            metadata.ModifiedBy = CurrentUserName;
            metadata.ModifiedOn = Clock.Now;
        ...
        public void Update(string jsonData, bool isSubmit)
        {
            ...
            VisionCalibrationConfig metadata = JsonConvert.DeserializeObject<VisionCalibrationConfig>(jsonData, JsonSetting);
            Update(metadata);
        }
        ...
        public void CheckOut(Guid metadataId)
        {
            ...
            {
                // 签出运行时文件，生成设计时文件
                VisionCalibrationConfig metadata = FileOperator.CheckOut(metadataId);
                // 刷新二开运行时
                VisionCalibrationConfig runtimeMetadata = metadata.Clone();
                // 更新运行时缓存
        ...
        public void CreatRecipeFile(Guid OriginalGuid, Guid newGuid,out string targetPath)
        {
            ...
            VisionCalibrationConfig metadata = FileOperator.LoadMetadata(sourcePath);
            // 更新最后修改人，最后修改时间,Id
            metadata.ModifiedBy = CurrentUserName;
            metadata.ModifiedOn = Clock.Now;
            metadata.Id = newGuid;
        ...
        {
            VisionCalibrationConfig metadata;
        ...
        /// <returns></returns>
        public override MetadataStatusItem GetStatusItem(VisionCalibrationConfig metadata, StatusType status)
        {
