...
namespace DRsoft.Engine.Core.Vision.VisionProduction
{
    public abstract class MessageBase : TcpClient
    {
        ...
        /// 记录那些需要异步检测的消息
        /// </summary>
        public Dictionary<VisionProduction_Command, VisionMessageInfo> SendMassageDic;

        /// <summary>
        /// 记录那些需要同步反馈的消息
        /// </summary>
        public Dictionary<VisionProduction_Command, VisionMessageInfo> SyncMassageDic;
        ...
        public MessageBase()
        {
            ...
            Retry = 1;
            SendMassageDic = new Dictionary<VisionProduction_Command, VisionMessageInfo>();
            SyncMassageDic = new Dictionary<VisionProduction_Command, VisionMessageInfo>();
        }
        ...
        public virtual bool OpenConnection(string ip, int port)
        {
            if (ip == null || ip == "")
            {
                ...
                PubSubEventManager?.Publish(new EngineEventArgs
                    { Handle = DRSoftEventDefine.EVENT_VISIONPRODUCTION_ERR });
                return false;
            }
        ...
        /// <returns></returns>
        public virtual void SendMessage(VisionProduction_Command commandType, string sendMassage,
            string recvMessage = "")
        {
            try
            {
        ...
        /// <param name="recvMessage"></param>
        /// <returns></returns>
        public virtual void SendMessageSync(VisionProduction_Command commandType, string sendMassage,
            out string recvMessage)
        {
            try
            {
                recvMessage = "";
